    ...
    <table class="table table-striped table-hover">
        ...
        <tbody>
            ...
            <div class="modal fade" id="modal-{{ $demande->id }}" tabindex="-1" aria-labelledby="modalLabel-{{ $demande->id }}" aria-hidden="true">
              <div class="modal-dialog modal-lg" @if(app()->getLocale() == 'ar') dir="rtl" style="text-align: right;" @endif>
  ...
  @foreach($demande->documents ?? [] as $doc)
    ...
    @endforeach
                    ...
                    @if ($demande->status === 'terminee')
                      <h5 class="mt-4 text-success">✅ {{ __('app.translated_files') }} :

                      @if (!$demande->confirme_par_client)
                        <form method="POST" action="{{ route('demande.confirmer_reception', $demande->id) }}" class="d-inline">
                            @csrf
                            <button type="submit" class="btn btn-sm btn-outline-success" onclick="return confirm('{{ __('app.confirm_receipt_question') }}');">
                                ...
                                @foreach ($demande->messages as $message)
                                    <li class="list-group-item">
                                        <div class="d-flex justify-content-between">
                                            <span class="fw-bold"><i class="bi bi-chat-left-text"></i> {{ __('app.message') }}</span>
